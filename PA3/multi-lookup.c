...
*/
    ...
    if (check_args(argc, argv) < 0)
    {
        ...
        printf("%s: total time is %f seconds\n", argv[0], (stopTime.tv_nsec - startTime.tv_nsec) / 1000000000.0 + stopTime.tv_sec - startTime.tv_sec);
        return -1;
    }
    ...
    while (1)
    {
        ...
        while (testStkEmpty(sharedStk))
        {
            if (sharedStk->finishedAdding)
            {
                ...
                pthread_cond_signal(&cDomainAvail); // The first one to finish has to unblock the rest
                return NULL;
            }
        ...
        char ipAddress[MAX_IP_LENGTH]; // Make a spot for the ip address

        if (dnslookup(domainName, ipAddress, sizeof(ipAddress)) == UTIL_FAILURE)
        {
            solveCount -= 1;
            fprintf(stderr, "DNS lookup could not find domain name: %s\n", domainName);
            strcpy(ipAddress, "NOT_RESOLVED");
        }
